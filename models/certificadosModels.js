...
module.exports = {
  ...
  create: async function (data) {
    const {
      ...
      url,
      fecha_finalizacion,
      ruta_foto_certificado,
    } = data;

    console.log("url llego", url);

    const insertQuery =
      "INSERT INTO certificados (nombre_certificado,descripcion,url_certificado,ruta_foto_certificado,fecha_fin_certificado) values ($1,$2,$3,$4,$5)";
    try {
  ...
  actualizar: async function (data) {
    const { id_cer, nombre, descripcion, fecha_finalizacion, url, ruta_Foto } =
      data;
  ...
  getByNombre: async function (nombrePagina) {
    const results = await pool.query(
      "select * from certificados where nombre_certificado = $1",
      [nombrePagina]
    );
    return results.rows;
  },
